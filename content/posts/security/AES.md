...
在计算机中，A(x)可以用向量表示(a7,a6,a5,a4,a3,a2,a1,a0), 而 x^k 这样的因此并不需要存储.
...
$$
\begin{align*}
A(x)-B(x) &= x^7 + x^6 + x^4 + 1 - x^4 - x^2 - 1 \\
&= x^7 + x^6 - x^2 \\
&= x^7 + x^6 - x^2 + 2 * x^2 \\
&= x^7 + x^6 + x^2
\end{align*}
...
> 
> AES标准中， 作者设计了一个不可约多项式 `P(x) = x^8 + x^4 + x^3+ x^1 + 1`，来进行模约.
